...
func main() {
	...
	for _, processor := range c.Clusters {
		...
		opts = append(opts, elastic.SetURL(processor.Output.ElasticSearch.Hosts...))
		opts = append(opts, elastic.SetGzip(processor.Output.ElasticSearch.Compress))
		if len(processor.Output.ElasticSearch.Username) > 0 && len(processor.Output.ElasticSearch.Password) > 0 {
			opts = append(opts, elastic.SetBasicAuth(processor.Output.ElasticSearch.Username, processor.Output.ElasticSearch.Password))
		}
